...
const TestimonialCard = ({ t, highlight }) => (
  <div className={`p-6 rounded-2xl border bg-white ${highlight ? 'border-emerald-200' : 'border-white/70'} shadow-sm`}>
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-3">
        <div className="text-amber">★★★★★</div>
      </div>
      {t.badge && <div className="text-xs bg-emerald-50 text-emerald-800 px-3 py-1 rounded-full">{t.badge}</div>}
      {t.badgeAlt && <div className="text-xs bg-emerald-50 text-emerald-800 px-3 py-1 rounded-full">{t.badgeAlt}</div>}
    </div>

    <blockquote className="mt-4 italic text-sm leading-relaxed">“{t.quote}”</blockquote>

    <div className="mt-6 flex items-center gap-3">
      <div className="h-10 w-10 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold">{t.name.charAt(0)}</div>
      <div>
      ...
      <div className="container max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <TestimonialCard key={i} t={t} highlight={i % 3 === 0} />
          ))}
        </div>
      </div>
    </section>
