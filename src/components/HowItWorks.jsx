  ...
  return (
    <section className="py-16 bg-emerald-50">
      <div className="container max-w-7xl mx-auto px-6 text-center">
        ...
        <div className="inline-flex rounded-xl border mt-6 overflow-hidden mx-auto">
          <button onClick={() => setMode('buyer')} className={`px-6 py-3 ${mode==='buyer' ? 'bg-olx-teal text-white' : 'bg-white text-olx-dark'}`}>For Buyers</button>
          <button onClick={() => setMode('seller')} className={`px-6 py-3 ${mode==='seller' ? 'bg-olx-teal text-white' : 'bg-white text-olx-dark'}`}>For Sellers</button>
        </div>

        <div className="mt-10 grid grid-cols-1 sm:grid-cols-4 gap-6 items-start">
          {steps.map((s, i) => (
            <div key={i} className="text-left">
              <div className="h-12 w-12 rounded-full bg-white border flex items-center justify-center mx-auto mb-4">{i+1}</div>
